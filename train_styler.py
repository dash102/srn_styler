...
p.add_argument('--clip_weight', type=float, default=1,
               help='Weight for style regularization.')
p.add_argument('--style_weight', type=float, default=1,
               help='Weight for style regularization.')
...
p.add_argument('--appearance_embedding_size', type=int, default=128,
               help='Dimensionality of latent embedding.')

# clip constraints
p.add_argument("--description", type=str, default="car")
p.add_argument("--prefix", type=str, default="A model of a")
p.add_argument("--suffix", type=str, default="")

p.add_argument("--use_clip", action='store_true', help='whether use clip loss')
p.add_argument("--use_alpha", action='store_true', help='whether finetune alpha layers')
p.add_argument("--use_feature", action='store_true', help='whether finetune feature layers')
p.add_argument("--use_view", action='store_true', help='whether finetune view layers')
p.add_argument("--w_clip", type=float, default=1., help='weight for CLIP loss')
p.add_argument("--sample_scale", type=int, default=60, help='sample scale for patch-based samplar')
...
p.add_argument("--cache_dir", type=str, default="../clip_models",)
p.add_argument("--pos_embed", action='store_true')
...
def train():
    ...
    model = SRNsModel(num_instances=train_dataset.num_instances,
                      ...
                      freq_bands=opt.multires,
                      appearance_dim=opt.appearance_embedding_size,
                      text_emb_dim=512,
    ...
    # CLIP definitions
    with torch.no_grad():
        tokenizer = model.style_loss.tokenizer
        clip_model = model.style_loss.model
        prompt = opt.prefix + ' ' + opt.description + ' ' + opt.suffix
        print('PROMPT:', prompt)
        text_inputs = tokenizer([prompt], padding=True, return_tensors="pt")
        text_features = clip_model.get_text_features(**{k: v.cuda() for k, v in text_inputs.items()})

    print('POSITION EMBEDDING:', opt.pos_embed)
    ...
    for img_sidelength, max_steps, batch_size in zip(img_sidelengths, max_steps_per_sidelength,
        ...
        while True:
            for model_input, ground_truth in train_dataloader:
                model_outputs = model(model_input, text_features)
                ...
                img = model.get_output_img(model_outputs)
                style_loss = model.get_style_loss(img, [prompt])
                ...
                weighted_style_loss = opt.style_weight * style_loss
                ...
                print("Iter %07d   Epoch %03d   L_img %0.4f   L_latent %0.4f   L_depth %0.4f   L_style %0.4f" %
                      (iter, epoch, weighted_dist_loss, weighted_latent_loss, weighted_reg_loss, weighted_style_loss))
                ...
                writer.add_scalar("scaled_latent_loss", weighted_latent_loss, iter)
                writer.add_scalar("scaled_style_loss", weighted_style_loss, iter)
