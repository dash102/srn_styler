...
class Mapper(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.module(x_projected) + x_projected
...
class SRNsModel(nn.Module):
    def __init__(self,
        ...
        self.num_hidden_units_phi = 256 + appearance_dim
        ...
        self.embed_fn, self.in_ch = get_embedder(freq_bands, int(pos_embed) - 1)
        self.deform_net = DeformationModel(self.in_ch).train().cuda()
        self.appearance_mapper = Mapper(text_emb_dim)
        self.shape_mapper = Mapper(text_emb_dim)
        ...
        if use_unet_renderer:
            ...
            self.pixel_generator = pytorch_prototyping.FCBlock(hidden_ch=self.num_hidden_units_phi,
                                                               ...
                                                               in_features=self.num_hidden_units_phi + appearance_dim,
                                                               out_features=3,
        ...
        self.style_loss = StyleScorer(cache_dir).cuda()
    ...
    def get_style_loss(self, images, text):
        return self.style_loss(images, text).mean(dim=0)
    ...
    def forward(self, input, text_emb, z=None):
        ...
        points_xyz_emb = self.embed_fn(points_xyz)

        appearance_code = self.appearance_mapper(text_emb)
        shape_code = self.shape_mapper(text_emb)
        shape_code = shape_code.expand((*points_xyz.shape[:-1], shape_code.shape[-1]))
        deformation_delta = self.deform_net(torch.cat([points_xyz, shape_code], dim=-1))
        model_input_deformed = points_xyz_emb + deformation_delta

        # Sapmle phi a last time at the final ray-marched world coordinates.
        # v = phi(points_xyz)
        # novel_views = self.pixel_generator(v)

        v = phi(model_input_deformed)
        appearance_code = appearance_code.expand((*v.shape[:-1], appearance_code.shape[-1]))
        v = torch.cat([v, appearance_code], dim=-1)
        ...
        with torch.no_grad():
            ...
            self.logs.append(("image", "normals",
                              torchvision.utils.make_grid(normals, scale_each=True, normalize=True), 1))
